  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <p className="text-gray-400 mb-12">
        ...
        {analysis?.summary && (
          <div className="mb-12 p-6 border border-gray-700 rounded-sm">
            <h3 className="font-medium text-white mb-3">Анализ сайта</h3>
            <p className="text-gray-400 leading-relaxed">{analysis.summary}</p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-12">
          {STYLE_OPTIONS.map(style => (
            <button
              ...
              className={`p-6 border rounded-sm transition-all text-left ${
                selectedStyles.includes(style.id)
                  ? 'border-white bg-white bg-opacity-10'
                  : 'border-gray-700 hover:border-gray-500'
              }`}
            >
              <h3 className="font-medium text-white mb-2">{style.name}</h3>
        ...
        <div className="flex justify-center">
          <button
            onClick={onGenerate}
            className="px-16 py-4 bg-white text-black font-medium text-lg rounded-sm hover:bg-gray-200 transition-colors"
          >
